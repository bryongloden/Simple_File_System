 ...
 * data blocks  1 block each until end of disk image
 * 
 * Notes:
 * The first inode is 2.
 */
...
#define bitmap_mask0 0x80
#define bitmap_mask1 0x40
#define bitmap_mask2 0x20
#define bitmap_mask3 0x10
#define bitmap_mask4 0x08
#define bitmap_mask5 0x04
#define bitmap_mask6 0x02
#define bitmap_mask7 0x01
...
struct superblock {
    ...
    uint32_t s_first_data_block; /* which block is the first free data block */
...
extern void make_directory(char *path, unsigned int n); 
...
// n is the length of the string path
extern void rm_directory(char *path, unsigned int n);
// Make a new file of the specified size in the path provided.
// Initial data has to be passed in
// file contents are in data
// n is the length of the string path
extern void create_file(char *path, unsigned int n, unsigned int size, char *data);
...
// n is the length of the string path
extern void rm_file (char *path, unsigned int n);
// Make a hard link to the "*target" file at the "*path"
// n is the length of the string path
extern void make_link(char *path, unsigned int n, char *target);
...
//unsigned char inode_table[5*BLOCK_SIZE/sizeof(struct inode)];
